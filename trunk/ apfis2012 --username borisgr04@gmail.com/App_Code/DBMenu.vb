...
Public Class DBMenu
    ...
    Public Overloads Sub cargarMenu(ByVal mn As Menu, ByVal Modulo As String)
        ...
        'Throw New Exception(Conexion.ConnectionString)
    ...
    Public Sub cargarElementos(ByRef tvw As TreeView, ByVal perfil As String, Optional ByVal Modulo As String = "ADMIN")
        ...
        Me.Conectar()
        'se invoca al procedimiento almacenado
        queryString = "select (select count(*) from vMenu_Perfil Where menuid=m.menuid AND sisperfil=:perfil ) As Est, m.* From vmenu m where MODULO=:MODULO Order by menuid,posicion"
        Me.CrearComando(queryString)
        Me.AsignarParametroCadena(":perfil", perfil)
    ...
    Private Sub AddMenuItem(ByRef mnuMenuItem As TreeNode, ByVal dtMenuItems As Data.DataTable)
        'recorremos cada elemento del datatable para poder determinar cuales son elementos hijos
        'del menuitem dado pasado como parametro ByRef.
        For Each drMenuItem As Data.DataRow In dtMenuItems.Rows
            If drMenuItem("PadreId").ToString.Equals(mnuMenuItem.Value) AndAlso _
    ...
    Private Function AgregarPerfil(ByVal lst As String, ByVal nomper As String) As String
        ...
        Try
            For k = 0 To nL - 1
                ...
                'Dim par As BDParametros = New BDParametros("inMNID", DbType.String, vMenu(k), ParameterDirection.Input)
                'Me.AgregarParametro(par)
                'par = New BDParametros("inNOMPER", DbType.String, nomper, ParameterDirection.Input)
                'Me.AgregarParametro(par)
